    ...
    <div class="d-flex flex-col flex-grow scroll-hide">
        <header-layout>
            <div class="row row-collapse w-100">
                ...
                <div class="col col-vmid">
                    <h3 class="last text-bold">Редактировать</h3>
                </div>
                <div class="col col-auto">
        ...
        <content-layout>
            <div class="d-flex flex-col flex-h-center pad-5">
                ...
                <button @click="updateGroup" class="btn btn-primary">Сохранить</button>
            </div>
        </content-layout>
    </div>
</template>
...
export default {
    ...
    methods: {
        async getInfo() {
            let data = (await this.$groupApi.getGroupInfo(this.$route.params.id)).info;
            this.name = data.name;
            this.desc = data.description;
            this.isHidden = Boolean(Number(data.is_hidden));
            this.isPublic = Boolean(Number(data.is_public));
            this.allowExit = Boolean(Number(data.allow_exit));
        },
        async updateGroup() {
            if (!this.name) {
            ...
            try {
                ...
                EventBus.$emit(EventBusEvent.SHOW_TOAST, 'Group updated');
                this.$router.push('/group/' + data.group_id);
            } catch {
                EventBus.$emit(EventBusEvent.SHOW_TOAST, 'Group update failed');
            }
        }
    },
    activated() {
